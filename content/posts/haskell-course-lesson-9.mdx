...
**Welcome to the first lesson where we're moving beyond just GHCi and creating our first Haskell project!** If you did last lesson's homework, you are ready for this one! No time to waste! Let's go!! 🔥
...
To load a file into GHCi, we can use the `:l` (load) command and specify the path to the file—in our case: `:l lesson_9.hs`—and hit Enter.
...
<BlockNote type='success'>
  <p className='mt-0'>
    Notice that—because there's no other file—you can write <code>:l le</code>
...
Remember that each time you change the file and want to test it, you have to save the file (Ctrl+S on Windows/Linux and Command+S on MacOs) and reload it with `:l`.
...
So, what could we do? One solution is to use indentation.
...
Ok, now we're ready to write some code. Next, we'll create a few functions that work together to create the blockchain-like data structure we saw in <a target="_blank" href='https://robertinom.com/posts/haskell-course-lesson-6#designing-a-blockchains-data-structure'>lesson 6 - Tuples</a>.
...
(
    ...
    ,[
       ...
       ,("Jeff","Sahra", 92, "f4c655bbe173b37b832f1439e947fa29")
...
<BlockNote type='success'>
  <div>
    <p className='mt-0'>
      We can use <code>show</code> to transform to <code>String</code> any type
      that it's an instance of the <code>Show</code> type class. For example, to
      concatenate a <code>String</code> with a number, we first transform that
...
<Accordion
  title={
    <div>
      ...
      (two strings representing the addresses of the sender and receiver and the
      value of the transaction) and concatenates all of them to a single{' '}
      <code>String</code>.
    </div>
  }
  content={
...
Now let's create a transaction's data structure!
...
<Accordion
  title={
    <div>
      ...
      (two strings representing the addresses of the sender and receiver and the
      value of the transaction) that generates a 4-element tuple with all those
      values plus the ID of the transaction.
    </div>
  }
  content={
    <div>
...
```haskell
*Main> tx1 = createTx "Sahra" "Carl" 100
*Main> tx1
("Sahra","Carl",100,"SahraCarl100")
```
...
<Accordion
  title={
    <div>
      Write a function named <code>createBlock</code> that takes a string
      representing the previous block ID with a list of transactions and creates
      a block with the structure we're aiming for. (
      <a
        target='_blank'
      ...
      structure.) Generate the block ID by concatenating the previous block ID
      with the number of transactions contained in the block.
    </div>
  }
  content={
    <div>
...
Congratulations! 🥳 We have everything we need to create mockup blockchain blocks with the structure we want!
...
Obviously, this implementation isn't viable for a real blockchain (it doesn't actually do anything but assemble the structure). But served as a great way to introduce Haskell files and how to use them. Especially now that you have all those functions with syntax highlighting at a glance and ready to be improved!

<p>
  <mark>
    The next lesson will be about my favorite Haskell feature: Pattern Matching!
    🤩 That's when you can start to feel the actual power of Haskell! 💪 See you
    then!
  </mark>
</p>
